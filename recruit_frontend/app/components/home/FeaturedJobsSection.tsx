  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-4 py-8 md:py-10">
      <div className="mb-4 flex items-center justify-between">
        ...
        <Link href="/auth/login" className="text-sm font-medium text-slate-700 hover:underline">
          Xem toàn bộ
        </Link>
      </div>
        ...
        {jobs.map((job) => (
          <article key={job.id} className="rounded-lg border border-slate-200 bg-white p-4 transition-colors hover:border-slate-300">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-base font-semibold leading-6">{job.tieuDe}</h3>
                  <span className="rounded border border-slate-300 bg-slate-100 px-2 py-0.5 text-xs font-medium">{job.tag}</span>
                </div>
                <p className="mt-1 text-sm text-slate-600">{job.congTy}</p>
                <p className="text-xs text-slate-500">{job.diaDiem}</p>
              </div>
              <button type="button" className="rounded-md border border-slate-300 px-3 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-100">
                Ứng tuyển nhanh
              </button>
            </div>
            <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-600">
              ...
              <span className="rounded bg-slate-100 px-2 py-1">Mã: {job.id}</span>
            </div>
          </article>
        ))}
      </div>
