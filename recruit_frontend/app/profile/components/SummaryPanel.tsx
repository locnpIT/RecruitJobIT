...
export function SummaryPanel({
  ...
  saving: boolean;
  onChange: (next: { tenHoSo: string; gioiThieuBanThan: string; mucTieuNgheNghiep: string }) => void;
  onSave: () => void;
}) {
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5">
      ...
      <div className="mt-4 grid gap-3">
        <input
          value={tenHoSo}
          onChange={(e) => onChange({ tenHoSo: e.target.value, gioiThieuBanThan, mucTieuNgheNghiep })}
          placeholder="Tên hồ sơ (VD: Backend Developer)"
          className="h-10 w-full rounded-md border border-slate-300 px-3 text-sm"
        />
        <textarea
          rows={4}
          value={gioiThieuBanThan}
          onChange={(e) => onChange({ tenHoSo, gioiThieuBanThan: e.target.value, mucTieuNgheNghiep })}
        ...
        <textarea
          ...
          onChange={(e) => onChange({ tenHoSo, gioiThieuBanThan, mucTieuNgheNghiep: e.target.value })}
